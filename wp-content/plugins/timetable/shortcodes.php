...
function tt_event_item($atts, $content)
{
	...
	<li' . ($type=="info" ? ' class="timetable_clearfix type_info"' : '') . ($border_color!='' ? ' style="border-bottom: ' . ($border_color=='none' ? 'none' : '1px solid #' . $border_color . '') . ';"' : '') . '>
		<' . ($type=="info" ? 'label' : 'span') . ($text_color!='' ? ' style="color: #' . $text_color . ';"' : '') . '>' . remove_wpautop($content) . '</' . ($type=="info" ? 'label' : 'span') . '>';
		if($value!="")
...
function tt_event_columns($atts, $content)
{
	...
	return '<div class="tt_event_columns' . ($class!='' ? ' ' . $class : '') . '">' . remove_wpautop($content) . '</div>';
}
...
{
	return '<div class="tt_event_column_left">' . remove_wpautop($content) . '</div>';
}
