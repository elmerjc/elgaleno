...
function timetable_enqueue_scripts()
{
	...
	wp_enqueue_style('timetable_font_lato', 'https://fonts.googleapis.com/css?family=Lato:400,700');
}
...
if(is_admin())
{
	...
	{
		$page = add_options_page('Timetable', 'Timetable', 'administrator', 'timetable_admin', 'timetable_admin_page');
	...
	function timetable_admin_page()
	{
		...
		{
			$events_select_list .= '<option value="' . $event->post_name . '">' . $event->post_title . ' (id: ' . $event->ID . ')' . '</option>';
		...
		foreach($events_categories as $events_category)
			$events_categories_list .= '<option value="' . esc_attr($events_category->slug) . '">' . $events_category->name . '</option>';
		//weekdays string
		$weekdays_string = "";
		$weekdays_select_list = "";
		foreach($weekdays_list as $weekday)
		{
			$weekdays_select_list .= '<option value="' . $weekday->post_name . '">' . $weekday->post_title . ' (id: ' . $weekday->ID . ')' . '</option>';
...
function tt_timetable($atts, $content)
{
	...
	$events_list_html = '<li><a href="#all-events" title="' . esc_attr($filter_label) . '">' . $filter_label . '</a></li>';
	...
	if($font!="")
			$output .= '<link rel="stylesheet" type="text/css" href="https://fonts.googleapis.com/css?family=' . $font . '&subset=' . $font_subset . '">';
	...
	$output .= '<div id="all-events">' . tt_get_timetable($atts, $events_array) . '</div>';

	if($filter_kind=="event" || !count($event_category_array))
	{
		for($i=0; $i<$events_array_count; $i++)
			$output .= '<div id="' . $events_array[$i] . '">' . tt_get_timetable($atts, $events_array[$i]) . '</div>';
	}
	else
	{
		for($i=0; $i<$events_category_array_count; $i++)
		{
			...
			$output .= '<div id="' . $event_category_array[$i] . '">' . (count($events_array) ? tt_get_timetable($atts, $events_array_for_timetable) : __('No events available in ' . $event_category_array[$i] . ' category!', 'timetable')) . '</div>';
		}
	}
	$output .= '</div>';
...
function to_decimal_time($time, $midReplace = false)
{
	...
	return ($midReplace && (int)$timeExplode[0]==0 ? 24 : $timeExplode[0]) . "." . ((int)$timeExplode[1]>0 ? ceil($timeExplode[1]/60*100) : "00");
}
...
function tt_get_row_content($events, $events_page, $time_format, $event_layout, $global_colors, $disable_event_url)
{
	...
	foreach($events as $key=>$details)
	{
		...
		$class_link = '<' . ($classes_url!="" ? 'a' : 'span') . ' class="event_header"' . ($classes_url!="" ? ' href="' . $classes_url /*. '#' . urldecode($details["name"])*/ . '"' : '') . ' title="' .  esc_attr($key) . '"' . ($text_color!="" ? ' style="color: #' . $text_color . ' !important;"' : '') . '>' . $key . '</' . ($classes_url!="" ? 'a' : 'span') . '>';

		for($i=0; $i<$hours_count; $i++)
		{
			$tooltip = "";
			$content .= '<div class="event_container' . (count(array_filter(array_values($details['tooltip']))) && (count($events)>1 || (count($events)==1 && $hours_count>1)) ? ' tt_tooltip' : '' ) . '"' . ($color!="" || ($text_color!="" && (count($events)>1 || (count($events)==1 && $hours_count>1))) ? ' style="' . ($color!="" ? 'background-color: #' . $color . ';' : '') . ($text_color!="" && (count($events)>1 || (count($events)==1 && $hours_count>1)) ? 'color: #' . $text_color . ';' : '') . '"': '') . (($hover_color!="" || $hover_text_color!="" || $hours_hover_text_color!="") && (count($events)>1 || (count($events)==1 && $hours_count>1)) ? ' onMouseOver="' . ($hover_color!="" ? 'this.style.background=\'#'.$hover_color.'\';' : '') . ($hover_text_color!="" ? 'this.style.color=\'#'.$hover_text_color.'\';jQuery(this).find(\'.event_header\').css(\'cssText\', \'color: #'.$hover_text_color.' !important\');' : '') . ($hours_hover_text_color!="" ? 'jQuery(this).find(\'.hours\').css(\'color\',\'#'.$hours_hover_text_color.'\');' : '') . '" onMouseOut="' . ($hover_color!="" ? 'this.style.background=\'#'.$color.'\';' : '') . ($hover_text_color!="" ? 'this.style.color=\'#'.$text_color.'\';jQuery(this).find(\'.event_header\').css(\'cssText\',\'color: #'.$text_color.' !important\');' : '') . ($hours_hover_text_color!="" ? 'jQuery(this).find(\'.hours\').css(\'color\',\'#'.$hours_text_color.'\');' : '') . '"' : '') . '>';
...
function tt_get_timetable($atts, $event = null)
{
	...
	if((int)$measure==1)
	{
		...
		$max_hour = (int)$hours->max + ( isset($max_explode[1]) && (int)$max_explode[1]>0 ? 1 : 0 );
	}
	else
	{
		$max_hour = $hours->max;
			...
			if(!in_array($weekday_fixed_number, (array)(isset($drop_columns[$i]["columns"]) ? $drop_columns[$i]["columns"] : array())))
			{
				if(tt_hour_in_array($i, (isset($event_hours_tt[$weekday_fixed_number]) ? $event_hours_tt[$weekday_fixed_number] : array()), $measure, $hours_min))
				{
					...
					for($k=0; $k<$array_count; $k++)
					{
						...
						{
							$events[$event_hours_tt[$weekday_fixed_number][$k]["title"]]["name"] = $event_hours_tt[$weekday_fixed_number][$k]["name"];
							$events[$event_hours_tt[$weekday_fixed_number][$k]["title"]]["tooltip"][] = $event_hours_tt[$weekday_fixed_number][$k]["tooltip"];
							$events[$event_hours_tt[$weekday_fixed_number][$k]["title"]]["before_hour_text"][] = $event_hours_tt[$weekday_fixed_number][$k]["before_hour_text"];
							$events[$event_hours_tt[$weekday_fixed_number][$k]["title"]]["after_hour_text"][] = $event_hours_tt[$weekday_fixed_number][$k]["after_hour_text"];
							$events[$event_hours_tt[$weekday_fixed_number][$k]["title"]]["id"] = $event_hours_tt[$weekday_fixed_number][$k]["id"];
							$events[$event_hours_tt[$weekday_fixed_number][$k]["title"]]["hours"][] = $event_hours_tt[$weekday_fixed_number][$k]["start"] . " - " . $event_hours_tt[$weekday_fixed_number][$k]["end"];
