...
function timetable_events_init()
{
	global $wpdb;
	$labels = array(
		'name' => _x('Events', 'post type general name', 'timetable'),
		'singular_name' => _x('Event', 'post type singular name', 'timetable'),
		'add_new' => _x('Add New', 'events', 'timetable'),
		'add_new_item' => __('Add New Event', 'timetable'),
		'edit_item' => __('Edit Event', 'timetable'),
		'new_item' => __('New Event', 'timetable'),
		'all_items' => __('All Events', 'timetable'),
		'view_item' => __('View Event', 'timetable'),
		'search_items' => __('Search Event', 'timetable'),
		'not_found' =>  __('No events found', 'timetable'),
		'not_found_in_trash' => __('No events found in Trash', 'timetable'),
		'parent_item_colon' => '',
		'menu_name' => __("Events", 'timetable')
	);
	$args = array(
		"labels" => $labels,
		"public" => true,
	...
	register_taxonomy("events_category", array("events"), array("label" => "Categories", "singular_label" => "Category", "rewrite" => true));
...
function timetable_save_events_postdata($post_id)
{
	...
	if ( !isset($_POST['timetable_events_noncename']) || !wp_verify_nonce($_POST['timetable_events_noncename'], plugin_basename( __FILE__ )) )
		return;
...
add_action("manage_events_posts_custom_column", "manage_events_posts_custom_column");
?>